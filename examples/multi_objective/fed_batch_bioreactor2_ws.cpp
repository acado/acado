 ...
 */
 ...
 /**   
  *    \file   examples/multi_objective/fed_batch_bioreactor2_ws.cpp
...
int main( ){
    ...
    algorithm.getWeights("fed_batch_bioreactor2_ws_weights.txt"); 
    algorithm.getAllDifferentialStates("fed_batch_bioreactor2_ws_states.txt");
    algorithm.getAllControls("fed_batch_bioreactor2_ws_controls.txt");
    algorithm.getAllParameters("fed_batch_bioreactor2_ws_parameters.txt");
    ...
    // SAVE INFORMATION:
    // -----------------
    FILE *file = fopen("fed_batch_bioreactor2_ws_pareto.txt","w");
    paretoFront.print();
    file << paretoFront;
    fclose(file);
