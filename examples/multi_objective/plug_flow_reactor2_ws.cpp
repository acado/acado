 ...
 */
 ...
 /**   
  *    \file   examples/multi_objective/plug_flow_reactor2_ws.cpp
...
int main( ){
    ...
    algorithm.set( KKT_TOLERANCE, 1e-11 );
    ...
    algorithm.getWeights("plug_flow_reactor2_ws_weights.txt");
    algorithm.getAllDifferentialStates("plug_flow_reactor2_ws_states.txt");
    algorithm.getAllControls("plug_flow_reactor2_ws_controls.txt");
    ...
    // SAVE INFORMATION:
    // -----------------
    FILE *file = fopen("plug_flow_reactor2_ws_pareto.txt","w");
    paretoFront.print();
    file << paretoFront;
    fclose(file);
