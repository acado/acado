...
class MyFunction : public UserFunction<double,MyFunction>{   // inherit form UserFunction;
...
public:
    ...
    typedef UserFunction<double,MyFunction> Base;
  
    MyFunction() : Base(2){ }  // we may set the output dimension
                               // in the constructor as it
                               // is not allowed to change anyhow.
  
    MyFunction( const MyFunction &rhs ):Base(rhs){ }
  
    virtual MyFunction* clone() const{ return new MyFunction(*this); }
    ...
    virtual void evaluate( const std::vector<double> &input,
       ...
       output[1] = input[0];                     // define what-ever you like!
    }
};
