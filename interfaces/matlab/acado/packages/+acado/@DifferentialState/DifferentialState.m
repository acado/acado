            ...
            for j = 1:length(obj)
                for i = 1:length(var)
                    if isa(var(i), 'acado.Variable')
                        jac(j,i) = double(isa(var(i), 'acado.DifferentialState') && strcmp(obj(j).name, var(i).name));
                    else
                        if isa(var(i).obj1, 'acado.Variable')
                            jac(j,i) = double(isa(var(i).obj1, 'acado.DifferentialState') && strcmp(obj(j).name, var(i).obj1.name));
                        else
