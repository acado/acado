...
SharedOperator Addition::substitute( SharedOperatorMap &sub ){
    ...
    return SharedOperator( new Addition( a1->substitute( sub ),
                                         a2->substitute( sub ) ) );
}

std::ostream& Addition::print( std::ostream &stream, StringMap &name ) const{
...
returnValue Addition::initDerivative() {

    if( d1 != 0 ) return SUCCESSFUL_RETURN;

    d1  = SharedOperator( new DoubleConstant(1.0,NE_ONE)  );
    d2  = SharedOperator( new DoubleConstant(1.0,NE_ONE)  );
    d11 = SharedOperator( new DoubleConstant(0.0,NE_ZERO) );
    d12 = SharedOperator( new DoubleConstant(0.0,NE_ZERO) );
