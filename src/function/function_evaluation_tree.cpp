...
returnValue FunctionEvaluationTree::exportHeader(	FILE *file,
    ...
    if( getN() > 0 )
    	acadoFPrintf(file,"%s %s[%d];\n", realString, auxVariableName.getName(), nni );
...
returnValue FunctionEvaluationTree::exportCode(	FILE *file,
    ...
    //
    String auxVarName = "acadoWorkspace.";
    ...
    for( run1 = 0; run1 < n; run1++ )
    {
    	...
    	sub[ run1 ]->setVariableExportName(VT_INTERMEDIATE_STATE, auxVarName);

        acadoFPrintf(file,"acadoWorkspace.%s[%d] = ", auxVariableName.getName(), lhs_comp[run1] );
        file << *sub[run1];
        acadoFPrintf(file,";\n");
    }
    ...
    for( run1 = 0; run1 < dim; run1++ )
    {
    	...
    	f[ run1 ]->setVariableExportName(VT_INTERMEDIATE_STATE, auxVarName);
