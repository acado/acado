												...
												) const
{
	...
	exportHeader(stream, fcnName, realString);
	exportForwardDeclarations(stream, fcnName, realString);
	exportCode(stream, fcnName, realString, precision);
...
returnValue FunctionEvaluationTree::exportHeader(	std::ostream& stream,
													const char *fcnName,
													const char *realString
													) const{

    if( n > 0 )
    	stream	<< realString << " " << globalExportVariableName << "[" << n << "];" << endl;
...
returnValue FunctionEvaluationTree::exportCode(	std::ostream& stream,
    ...
    if (n > 0)
    {
    	stream	<< "/* Vector of auxiliary variables; number of elements:" << n << ". */" << endl
    			<< realString << "* a = " << globalExportVariableName << ";" << endl << endl
	...
	for (run1 = 0; run1 < n; run1++)
	{
		stringstream ss;
		ss << "a" << "[" << run1 << "]";
		auxVarIndividualNames.push_back( ss.str() );
	}
