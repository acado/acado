...
/**
 *    \file src/code_generation/integrators/irk_adjoint_export.cpp
...
AdjointIRKExport::AdjointIRKExport( const AdjointIRKExport& arg ) : ImplicitRungeKuttaExport( arg )
{
}


AdjointIRKExport::~AdjointIRKExport( )
{
	if ( solver )
		delete solver;
	solver = 0;
...
AdjointIRKExport& AdjointIRKExport::operator=( const AdjointIRKExport& arg ){
...
ExportVariable AdjointIRKExport::getAuxVariable() const
{
	ExportVariable max;
	if( NX1 > 0 ) {
...
returnValue AdjointIRKExport::getDataDeclarations(	ExportStatementBlock& declarations,
...
returnValue AdjointIRKExport::getFunctionDeclarations(	ExportStatementBlock& declarations
													) const
{
...
returnValue AdjointIRKExport::getCode(	ExportStatementBlock& code )
{
	int sensGen;
...
returnValue AdjointIRKExport::propagateOutputs(	ExportStatementBlock* block, const ExportIndex& index, const ExportIndex& index0, const ExportIndex& index1,
...
returnValue AdjointIRKExport::prepareInputSystem(	ExportStatementBlock& code )
{
	if( NX1 > 0 ) {
...
returnValue AdjointIRKExport::prepareOutputSystem(	ExportStatementBlock& code )
{
	if( NX3 > 0 ) {
...
returnValue AdjointIRKExport::sensitivitiesInputSystem( ExportStatementBlock* block, const ExportIndex& index1, const ExportIndex& index2, const ExportVariable& Bh, BooleanType STATES )
{
	if( NX1 > 0 ) {
...
returnValue AdjointIRKExport::sensitivitiesOutputs( ExportStatementBlock* block, const ExportIndex& index0,
...
returnValue AdjointIRKExport::setup( )
{
