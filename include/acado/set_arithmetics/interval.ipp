...
inline Interval operator*(const Interval&I1, const Interval&I2) {
	Interval I3(
			std::min(std::min(I1._l * I2._l, I1._l * I2._u),
...
inline double diam(const Interval &I) {
	return I._u - I._l;
}
inline double mid(const Interval &I) {
	return 0.5 * (I._u + I._l);
}
inline double abs(const Interval &I) {
	return std::max(::fabs(I._l), ::fabs(I._u));
}
...
inline Interval arh(const Interval &I, const double a) {
	Interval I2(::exp(-a / (a >= 0 ? I._l : I._u)),
			::exp(-a / (a >= 0 ? I._u : I._l)));
	return I2;
}
